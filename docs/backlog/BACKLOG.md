...
**STATUS: RESOLVED** — 2026-07-31: all four scenarios landed (Option B — per-service tests against a shared real broker; `docs/decisions/2026-07-31-payroll-disbursement-integration-harness.md`). **Scenarios 1–3** in integration-hub `PayrollDisbursementFlowTest` (real RabbitMQ + Postgres + Redis): (1) full happy path — approved → 2 COMPLETED → terminal `PaymentsCompletedEvent` (after-commit publish + JSON deserialization); (2) partial failure — one payslip's phone matches the sandbox fail prefix → 1 COMPLETED + 1 FAILED, counts split 1/1; (3) retry idempotency — `retryFailed` on a completed run re-dispatches nothing. **Scenario 4** in payroll-service `PayrollRunCompletionIdempotencyTest` (real RabbitMQ + Postgres): a duplicate `PaymentsCompletedEvent` completes the run at most once (`PayrollRun.complete()` idempotency, bug #4). **Finding:** the completion event is at-least-once at integration-hub (per-commit publish), effectively-once via the payroll-side guard. Shipped across PRs #133 (S1), #134 (S2), #137 (S3), #136 (S4).
