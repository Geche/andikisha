...
**STATUS: PARTIAL** — 2026-07-31: harness scaffolded; **Scenarios 1–3 landed**. Architecture decided (Option B — per-service tests against a shared real broker; `docs/decisions/2026-07-31-payroll-disbursement-integration-harness.md`). `PayrollDisbursementFlowTest` (integration-hub) boots real RabbitMQ + Postgres + Redis. **Scenario 1 (happy path):** `PayrollApprovedEvent` → both transactions COMPLETED → terminal `PaymentsCompletedEvent` (exercises after-commit publish + JSON deserialization). **Scenario 2 (partial failure):** one payslip's phone matches the sandbox fail prefix → one COMPLETED + one FAILED, terminal counts split 1/1. **Scenario 3 (retry idempotency):** `retryFailed` on a fully-COMPLETED run re-dispatches nothing (conversation ids unchanged, H3 guard). **Finding:** integration-hub's `maybePublishRunCompleted` runs per transaction commit, so the completion event is **at-least-once, not exactly-once** — the payroll-side `PayrollRun.complete()` idempotency is what makes it effectively-once. **Remaining scenario (its own PR):** (4) payroll-side — a duplicate `PaymentsCompletedEvent` completes the run only once (bug #4 idempotency), needing a real-broker harness in payroll-service.
...
**STATUS: RESOLVED** — 2026-07-30: `EmployeeQueryService.findAllActive()` (the gRPC `ListActiveByTenant` source for payroll runs and statutory filings) now excludes `pending_activation = true` rows via `findByTenantIdAndStatusInAndPendingActivationFalse`. Normal probation hires (`pending_activation = false`) stay eligible; only not-yet-activated bulk rows are excluded, and they re-enter once `BulkUploadService.activate` clears the flag. Verified by a Testcontainers repository test. **Residual — now also fixed (2026-07-30):** an *activated* employee missing a statutory ID (KRA PIN/SHIF/NSSF/phone) is excluded from the run via a completeness predicate (`PayrollService.missingStatutoryDetails`) and the run records an operator-facing warning ("N employee(s) excluded — incomplete statutory details: …") on `PayrollRun.notes`, now exposed in `PayrollRunResponse`. Skip-with-warning (not silent) per decision. **Remaining:** surface `notes` in the payroll-run UI (frontend follow-up).
