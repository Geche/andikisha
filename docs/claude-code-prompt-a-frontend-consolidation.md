...
The full rules of engagement are in `docs/design/template-usage.md` and the Claude Code skill at `.claude/skills/template-reference/SKILL.md`. Read both before starting Phase 2.

---

## Phase 0: Audit (read-only, no code changes)

Inspect each item below and produce a single Markdown report at `tenant-portal-consolidation-audit.md` in the repo root. Do not write any production code in this phase. The only file you create is the audit report.
...
- Compare against `andikishahr-brand-colours.md` in the project root
...
- Whether `docs/design/template-usage.md` exists
...
Produce `tenant-portal-consolidation-audit.md` with one section per audit task. End with a "Consolidation plan and risks" section listing:
...
Once Lawrence approves the audit, create a single file: `docs/adr/0001-single-tenant-portal.md`.
...
- **Template usage policy**: reference document at `docs/design/template-usage.md` defines the rules of engagement for the SmartHR template. Production code does not import from `template/*`.
...
When designing the layout chrome, consult `template/smarthr-html/` (or `template/smarthr-nextjs/`) for structural inspiration. Apply the three rules in `docs/design/template-usage.md`: visual reference only, no code copied, no template dependencies added. The chrome you build uses Tailwind, Lucide React, Bricolage Grotesque, DM Sans, and brand tokens from `andikishahr-brand-colours.md`.
...
- Uses the brand colours from `andikishahr-brand-colours.md`
...
Create `template/README.md` if it doesn't exist, with content explaining that the directory is read-only reference material, with the three rules of engagement, and pointing to `docs/design/template-usage.md` for the full reference.

Create `docs/design/template-usage.md` if it doesn't exist, using the content Lawrence supplies (delivered alongside this prompt).
...
9. See no remaining references to `admin-portal` or `employee-portal` in the repo (excluding `docs/adr/0001-single-tenant-portal.md` which documents the history).
10. See `frontend/landing/` unchanged.
11. See `template/README.md`, `docs/design/template-usage.md`, and `.claude/skills/template-reference/SKILL.md` in place.
...
After Phase 0, produce `tenant-portal-consolidation-audit.md` and stop. Wait for approval.
