...
5. **No `NavItem` export from `HorizontalShell`** using the canonical name from `menu-source-of-truth.md`. The doc calls it `NavItem`; the actual exported type is `HorizontalNavItem`. This naming divergence should be noted in the navConfig file comment.
