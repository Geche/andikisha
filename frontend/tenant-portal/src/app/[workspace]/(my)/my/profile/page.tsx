...
function Field({ label, value, icon: Icon }: { label: string; value: string | null | undefined; icon?: React.ElementType }) {
  return (
    <div className="flex items-start gap-3 py-3.5 border-b border-neutral-50 last:border-0">
      {Icon && (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11.5px] font-semibold text-neutral-400 uppercase tracking-wide mb-0.5">{label}</p>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 flex flex-col gap-5">
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-6 flex items-center gap-5">
          ...
          <div>
            {isLoading ? (
              <>
                <div className="h-4 w-40 bg-neutral-100 rounded-full animate-pulse mb-2"/>
